...
import SkillUpAd from "@/components/SkillUpAd";
    ...
    <div className="h-screen w-full">
      ...
      <Reveal variant="fadeUp" delay={0.25}>
        <section className="w-full flex flex-col gap-8">
          <QuickLinks />
          {latestNewsList.length !== 0 && <div className="relative w-full p-4 md:p-8 bg-[#f3f8fa] md:py-12">
            <h2 className="text-dark-primary text-xl md:text-[3xl] lg:text-[43px] font-medium text-center mb-4 md:mb-8">Latest News</h2>
            <div className="flex items-center justify-center gap-8">
              {latestNewsList.map((item:NewsPost, idx:number) => (
                <LatestNews key={idx} item={item} />
              ))}
            </div>
          </div>}
        </section>
